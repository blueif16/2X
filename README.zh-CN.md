...
2X 是一个住在 [OpenClaw](https://2x.infinityopus.com) 里的写作搭档。一天两次（或者随时），它看看你最近在忙什么，跟你聊一聊，帮你把想法打磨成值得发出去的东西。然后它处理机械部分——按平台排版、点按钮、发帖。

就这样。它不是自动驾驶。不会替你发帖。不会替你互动。不会优化触达率(但是可以选择监控)。就是一段 5 分钟的对话帮你把想法说清楚，然后分发出去。

## 一次典型的使用流程

Cron 触发（或者你说 `2x now`）。采集器静默收集你的智能痕迹——GitHub commit、浏览记录、聊天 session。然后它来找你。
...
1. Cron 触发采集器 → 收集你的智能痕迹 → **留在本地**
2. Agent 带着摘要来找你，聊什么有意思 → **留在本地**
...
![Session 草稿视图](assets/session-drafts-view.png)
...
![Supabase 认证通过](assets/supabase-auth-verified.png)

之后就是 `2x now` 或者等 cron 提醒。
...
[OpenClaw](https://2x.infinityopus.com) 生态的一部分。
